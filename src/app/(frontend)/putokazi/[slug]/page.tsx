...
export async function generateStaticParams() {
  const slugs = await fetchPutokaziSlugs();
  ...
  return (
    <div className="pattern-bg px-4 xl:px-10">
      <Container className="space-y-6 border-x border-dashed px-4 pt-32 pb-14 md:space-y-8 md:pt-40 md:pb-24">
        {cover ? (
          <div className="relative aspect-[21/9] min-h-[11rem] w-full max-h-80 overflow-hidden rounded-2xl border border-border/60 md:min-h-[14rem]">
            <Image
              src={cover.url}
              alt={cover.alt}
              fill
